...
2. Dentro de la carpeta *calidadaire*, crear un archivo llamado *docker-compose.yml*, como se muestra en la estructura:
...
El archivo [*docker-compose.yml*]((https://docs.docker.com/compose/overview/)) describe cada uno de los contenedores a desplegar así como instrucciones específicas como los puertos por donde se accederá, variables de entorno, si depende de algún otro servicio, comandos shell, etc.
...
La definición del archivo *docker-compose.yml* despliega los servicios indicados previamente:
...
4. Una vez que los servicios se levanten correctamente, podemos verificar su funcionamiento accediendo a ellos a través de los puertos indicados en el *docker-compose* y verificando que los servicios están ejecutandose con:
...
Para la creación de la entidad, realizamos un request **POST** a OCB definiendo la entidad con los atributos del modelo de datos de FIWARE [AirQualityObserved](https://fiware-datamodels.readthedocs.io/en/latest/Environment/AirQualityObserved/doc/spec/index.html).

[TODO]*****imagen POST grafana
...
[TODO imagen de patch o put]

Y la eliminación de una entidad

[TODO imagen de delete]

De ésta manera, nuestro servicio OCB se encuentra procesando la información de contexto que recibe manualmente a través de llamadas REST. Utilizando dispositivos de IoT, como [Cloudino](http://cloudino.io/) o [SmartSpot](http://smartcities.hopu.eu/smart-spot.html), la información se procesará de
...
[TODO ACTUALIZACOIN DE ENTIDAD] en OCB.
AirQualityUnit01

Verificamos el registro histórico de la entidad en QuantumLeap
```
http://localhost:8668/v2/entities/AirQualityUnit01
```

### API QuantumLeap

[TODO] NO NECESARIO AL MOMENTO
...
1. Para verificar los datos en la GUI a través de **http://localhost:4200** ubicamos la sección *Vista General* donde observamos las tablas creadas con el prefijo **et**. P. Ej. *etairqualityobsered*.
...
Para acceder a Grafana utilizamos las credenciales
